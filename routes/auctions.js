...
router.get("/auction/:_id", (req, res) => {
  var id = req.params._id;
  request.get(
    process.env.SERVER_API_URL + "/auctions/api/auction/" + id,
    (err, response, body) => {
      if (response.statusCode == 200) {
        var auction = JSON.parse(body);
        res.render("auction", {
          title: auction.name + " | Auction Away",
          auction: auction
        }); //single auction object
      } else {
        console.log(err);
...
router.post("/addauction", ensureAuthenticated, (req, res) => {
  ...
  if (errors) {
    res.render("addauction", {
      title: "Create Auction | Auction Away",
      errors: errors
    });
  } else {
    dayofauction = new Date(dayofauction + " GMT+0530");
    var dayofcreation = new Date(
      new Date(Date.now()).toDateString() + " GMT+0530"
    );
      ...
      if (user) {
        Auction.findById(auctionid, function(err, auction) {
          if (err) {
            ...
            if (
              bidamount > auction.bidprice &&
              user.balance >= auction.securitydeposit &&
              auction.state == 1
            ) {
